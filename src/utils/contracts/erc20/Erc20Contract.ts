import { ChainId, Erc20AssetIds } from '../../../types';
import { BaseContract } from '../BaseContract';

import defaultAbi from './abi.json';
import { ethers } from 'ethers';
import { getAssetAddress } from '../../../config/environment';
import { TransactionService } from '../../../services/transaction';

export class Erc20TokenContract extends BaseContract {
	assetId: Erc20AssetIds;
	...
	constructor(
		...
		super(chainId, getAssetAddress(chainId, asset), privateKey, abi);
		this.assetId = asset;
	}
	...
	approve = async (
		...
		await TransactionService.createErc20ApproveTransaction(
			tx.hash,
			spender,
	...
	transfer = async (
		...
		await TransactionService.createErc20TransferTransaction(
			tx.hash,
			toAddress,
			await this.formatValue(amount),
			this.chain,
	...
	transferFrom = async (
		...
		await TransactionService.createErc20TransferFromTransaction(
			tx.hash,
			fromAddress,
			toAddress,
			await this.formatValue(amount),
